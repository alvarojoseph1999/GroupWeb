...
if (!empty($_POST)) {
    ...
    $mensaje = "";
    $alert = "";
    if (empty($nombre) || empty($email) || empty($user)|| empty($descripcion)) {
        ...
        if (empty($id)) {
            ...
            if (empty($clave)) {
                ...
                if ($result > 0) {
                    ...
                    $query_insert = mysqli_query($conexion, "INSERT INTO usuario(nombre,correo,usuario,clave,descripcion) values ('$nombre', '$email', '$user', '$clave','$descripcion')");
                    if ($query_insert) {
            ...
            $sql_update = mysqli_query($conexion, "UPDATE usuario SET nombre = '$nombre', correo = '$email' , usuario = '$user', descripcion = '$descripcion', WHERE idusuario = $id");
            if ($sql_update) {
